...
## 2.9 2026-09-15 这一轮落地的能力（实测）

| 能力 | 状态 | 证据 |
| --- | --- | --- |
| 演示门禁（单口令，挡整个 API 面） | 已落地 | 未登录时 `POST /api/chat/stream` 返回 401；`test_session_gate.py` 遍历路由表做静态检查 |
| 补充查询（一次回答最多 2 条证据查询） | 已落地 | 实测触发「正在执行 2 条补充查询」；与主查询共用同一套 SQL 守卫 |
| 渲染产物持久化（切页面/刷新后图表仍在） | 已落地 | 存在独立表 `odoo_agent_message_artifacts`，一轮一行 |
| 执行链路记录 | 已落地 | 每条回答带 `trace_steps`，实测 11 步，SQL 生成 6.2s / 摘要 4.9s 是主要耗时 |
| 分时与缓存感知计价 | 机制已落地，**数值待填** | `DEEPSEEK_OFFPEAK_*` 默认不打折，行为与改动前一致 |

## 2.10 2026-09-15 修掉的缺陷（均有复现）

| 缺陷 | 影响面 | 根因 |
| --- | --- | --- |
| CTE 计算列被判「字段未开放」 | **所有同比/环比/占比查询** | 守卫把 CTE 别名映射回物理表，拿 CTE 的输出列去物理表白名单里找 |
| 「下降最大的三个月」被判未指定 Top N | 所有非「最高/最低」措辞的 Top-N | Top-N 识别只认 `最高|最低`，不认 `最大|最多|最快` 等 |
| 「莱茵重工的销售额」整轮失败 | **最自然的客户提问方式** | 过滤授权只看关键词表，问题里必须出现「客户」二字 |
| 造数把批次号写进客户/产品名 | 所有按名字的查询 | `seed_patterned_data.py` 给 name 加了 `（SYNTH-…）` 后缀；comment 和 origin 里本来就有，name 里那份是多余的 |
| 测试套件依赖开发者本机环境 | 任何新开的终端 / CI | 门禁口令在用户环境变量里，被测试进程继承，17 个用例 401 |
| Langfuse 注册表静默覆盖代码提示词 | **所有提示词修改** | `LANGFUSE_PROMPTS_FETCH_ENABLED` 默认为 true |
| 带中文的 .ps1 无 BOM | PowerShell 5.1 下直接语法错误 | 5.1 把无 BOM 的 UTF-8 当 ANSI 读 |

## 2.11 2026-09-15 补录的历史数据（实测）

**33 个月连续无缺口**（2024-01 ~ 2026-09），同比类问题现在算得出来。

| 年 | 订单 | 未税金额 | 同口径同比（1-9 月）|
| --- | ---: | ---: | ---: |
| 2024 | 524 | 8,866,401 | — |
| 2025 | 536 | 11,153,112 | +42.2% |
| 2026 | 466（至 09-09）| 8,517,223 | +11.6% |

- 造数脚本修了三处：`--end-month` 支持补录任意历史窗口；涨价因子**不再夹取到基准月**
  （原来 `max(months, 0)` 会让补录的历史全部停在基准月价格，涨价规律在时间轴上断开）；
  客户和产品按名字复用而不是重复创建。
- 涨价规律贯通已验证（固定产品逐年比）：伺服电机 M8 `1994.67 → 2461.80 → 3073.54`，
  六个常销产品全部单调递增，年均约 24~25%。
- 一次性写 1050 单会触发 PostgreSQL `OutOfMemory`（本机内存长期在 85% 以上）。
  改为每批 ≤3 个月、≤150 单后稳定。中断批次留下的草稿未清理——它们被
  `state IN ('sale','done')` 过滤掉，不影响任何销售指标，且与有意生成的 5% 异常数据
  无法区分。

## 2.12 2026-09-15 确定性部分纳入可观测（实测）

原来 Langfuse 里只看得到模型调用，"哪类问题在失败、占多少"答不上来。

| 新增 | 内容 |
| --- | --- |
| 拒绝理由分类码 | `SqlValidationResult.error_codes`，26 条规则映射到 `unlisted_table` / `filter_unauthorized` / `ranking_needs_top_n` 等稳定标签 |
| 意图路由观测 | `classify-question-intent` tool span，带判定结果和命中的词表 |
| 守卫观测 | 拒绝时附 `error_codes`，可直接聚合 |
| 轮次决策汇总 | 根节点带 `guard_error_codes` / `failed_stage` / `node_path` / `followup_count` / `elapsed_ms` |
| 执行链路面板 | 每条回答可展开，标红超过 3 秒的步骤；刷新后仍在 |

`test_sql_error_codes.py` 会遍历每一类拒绝场景断言分类码——**改了错误消息而忘记
同步 `_ERROR_CODES` 就会红**，不会悄悄退化成 `other`。

实测一条同比问题的耗时分布（总 148.1s）：

```
knowledge-retrieval  21.7s   （embedding + rerank 往返）
sql-generation       22.0s
chart-planning       36.5s
answer-synthesis     66.9s
其余 9 步            合计不到 1s
```

**147/148 秒在四次模型或外部 API 调用上**，本地计算几乎不占时间。这是下一步做延迟
优化的起点。

> 归属方向上踩过一次：阶段事件在**动手之前**发射，所以某一步的耗时是它到**下一步**
> 的间隔。最初写反了，把答案合成的 66 秒记成了"回答已完成"花了 66 秒，正好指错瓶颈。
> `test_turn_trace.py` 现在钉住这个方向。
