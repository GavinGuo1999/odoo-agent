...
| 未授权访问 Web | 演示门禁（单口令，挡整个 API 面）+ 仅监听 127.0.0.1 + Nginx IP 白名单 | 单口令无法区分用户，也没有审计到人；Token 只在进程内存里，重启即失效 |
| 别人拿你的 Key 烧钱 | 门禁默认保护**所有**路由，`POST /api/chat/stream` 未登录返回 401 | 拿到口令的人仍可无限次调用，没有配额 |
| Cloud 观测泄露数据 | 脱敏 Secret | 业务问题和结果仍会上传 |
| 状态库丢失 | PostgreSQL Checkpoint | 尚无会话备份/删除/保留期 UI |

## 10.1 演示门禁（2026-09-15 起）

给别人演示时，光在前端加登录框没有用：任何人 `POST /api/chat/stream` 依然能触发
模型调用、烧掉 API Key。所以校验在服务端，且**默认对所有路由生效**，只放行
`/api/auth/*` 本身。

- 口令**不以明文存在于任何地方**：存的是 PBKDF2-SHA256（600k 轮、每次不同盐）
  派生值，放在 `AGENT_UI_PASSWORD_HASH`。用 `configure-ui-password.ps1` 设置，
  明文只走 stdin，不进命令行参数。
- 留空即关闭门禁——本机开发保持零摩擦。
- 防爆破：同一来源连错 5 次锁 5 分钟。
- Token 存 sessionStorage（刷新还在、关标签页即失效），服务端只存进程内存，
  重启全部失效。
- `router.py` 里业务路由统一挂在一个带 `require_session` 的父 router 上，
  **新增路由默认就是受保护的**；`test_session_gate.py` 遍历整张路由表做静态检查，
  漏挂会红。
- `/api/health` 也在保护范围内：它会回显模型名、库名和订单数。
...
- 在**未启用门禁**的情况下暴露到公网（`AGENT_UI_PASSWORD_HASH` 为空即未启用）；
- 把门禁当成多用户权限系统——它只有一个口令，分不清是谁在用；
- 连接生产库高权限账号；
- 分析需要用户级权限隔离的数据；
- 让 Agent 执行写操作；
