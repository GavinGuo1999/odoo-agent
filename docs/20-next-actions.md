...
### P1-7 评测页的 RAGAS 显示会被后续运行盖掉——已完成，2026-09-15

`quality.html` 显示**最近一次** Wiki 运行的 RAGAS 状态。跑一次不带 `--ragas` 的检索评测后，
最近一次就没有 RAGAS 数据，页面显示"未运行"，而更早那次已建立的 Faithfulness 基线
（0.9075）不再可见。

页面本身没有错——它如实显示最近一次，也正确区分了"未运行"与"分数为 0"。问题在于
"最近一次"未必是"最近一次有 RAGAS 的"。

- 建议：`EvalReportService` 对 RAGAS 单独取"最近一条 `status != not_run` 的运行"，
  并在页面标注它来自哪一次，与检索指标的运行号分开显示。
- 验收：跑一次不带 `--ragas` 的评测后，页面仍能显示上一次的 Faithfulness 及其运行号。
...
### P1-5 切回 deepseek——已切换，2026-09-15（全量 A/B 复测仍待做）
...
### P1-6 给新增黄金集用例补参考结果签名——已完成（黄金集 76 题，pending_reference 清零）
...
### P1-8 提示词的 source of truth——已定，2026-09-15

`LANGFUSE_PROMPTS_FETCH_ENABLED` 默认值从 `true` 改为 `false`：**以代码为准**。

理由：默认为 true 时，Langfuse 注册表里的 production 版本会静默覆盖 `prompts.py`，
表现是"改了提示词完全不生效"且没有任何提示。代码是版本化、可评审、和测试同步的。
要用注册表就显式设为 `true`，并且要清楚那之后代码改动不再影响线上。

### P1-9 ECS 白名单随 IP 变动失效——已缓解，2026-09-15

工作站是家宽动态 IP（实测 9-10 太原 `171.116.31.154` → 9-15 上海 `112.64.39.228`），
变了就整站 403。已把当前 IP 写进 `odoo-stack/nginx-client-allowlist.conf`，
并新增 `odoo-stack/update-allowlist.sh`（在服务器上跑，默认取当前 SSH 来源 IP，
校验失败自动回滚）。

- **仍未做**：`issue-ip-https.sh` 从未执行，443 也没在安全组放行——HTTPS 一直没有启用。
  这两步都需要服务器访问权限。
...
| 事项 | 为什么还没做 |
| --- | --- |
| ECS 签发 IP 证书 + 安全组放行 443 | 需要 SSH/控制台权限，脚本已就绪 |
| 全量三轮 A/B 复测（deepseek） | 需要模型额度与约一小时，切换本身已完成 |
| ~~补 2024 年数据~~ | **已完成 2026-09-15**：2024-01 ~ 2026-09 共 33 个月连续无缺口 |
| 语义层的退款口径 | 8 个指标全部基于 `sale_order`，没有 `account_move`，退款不会从销售额里扣 |
| `D:\odoo19e\password.txt` 明文口令 | 删除属破坏性操作，留给用户决定；已确认无任何代码引用它 |
