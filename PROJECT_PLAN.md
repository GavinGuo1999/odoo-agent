...
1. 销售额是否采用“已确认订单的未税金额”。
2. 当前数据是否全部使用人民币；如果存在外币，需要先确定换算规则。
3. Langfuse 先使用 Cloud，还是本地 Docker 自托管。
4. 会话历史第一版使用本地 SQLite，还是单独的应用 PostgreSQL。
